...
def handle_search():
    data = request.json  # Expecting JSON data with a 'query' field
    if not data or 'query' not in data:
        return parse_json({'error': 'Missing query'}), 400

    query = data['query']

    try:
        results = search_query(query)  # Call your search function
        print(results)
        return parse_json({'data': results}),200
    except Exception as e:
